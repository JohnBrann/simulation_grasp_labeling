...
mat_path   = Sdf.Path("/World/Materials/ObjectMat")
object_mat = UsdShade.Material.Define(omni.usd.get_context().get_stage(), mat_path)

# 3) Author the friction & restitution on it
phys_api = UsdPhysics.MaterialAPI.Apply(object_mat.GetPrim())
phys_api.CreateStaticFrictionAttr().Set(0.5)   # e.g. medium grip
phys_api.CreateDynamicFrictionAttr().Set(0.4)
phys_api.CreateRestitutionAttr().Set(0.05)

# 4) (Optional) tweak PhysX‐only combine modes
px_api = PhysxSchema.PhysxMaterialAPI.Apply(object_mat.GetPrim())
px_api.CreateFrictionCombineModeAttr().Set("average")
px_api.CreateRestitutionCombineModeAttr().Set("average")

# 5) Bind it to your object’s collision prim
bind_api = UsdShade.MaterialBindingAPI.Apply(object.prim)
bind_api.Bind(
    object_mat,                            # the Material prim
)
